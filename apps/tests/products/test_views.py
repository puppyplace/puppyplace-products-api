...
class TestProductView:
    ...
    def test_create_product_succefully(self, client):
        data = {
            'title': 'Produto Demo sem categoria',
...
class TestProductDetailView:
    ...
    def test_update_product_succefully(self, client, products):
        ...
        response = client.patch(self.url.format(product.id), data={'title': new_product_title})
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_product_with_not_found_id(self, action, client):
        ...
        response = client_request(self.url.format(123), data={'name': 'new product name'})
